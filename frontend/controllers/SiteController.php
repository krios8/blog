...
class SiteController extends Controller
{
    ...
    public function actionCpost()
    {
        ...
        if ($posts->load(Yii::$app->request->post()) && $img->load(Yii::$app->request->post())) {
            $img->imageFile = UploadedFile::getInstance($img, 'imageFile');

            $imgi = $img->upload();
            $imgmodel->src = $imgi['src'];
            $imgmodel->name = $imgi['name'];
            $imgmodel->save();

            $posts->id_author = Yii::$app->user->identity->id;
            $posts->img_id = $imgmodel->id;
            $posts->save();

        }

        return $this->render('cpost', [
            'posts' => $posts,
            'ct' => $ct,
            'img' => $img
        ]);
    ...
    public function actionVpost($id)
    {
        $posts = new Posts();
        $post = $posts->find()->where(['id' => $id])->one();


        return $this->render('vpost', [
            'post' => $post
        ]);
    }
    public function actionUpost($id)
    {
        $posts = new Posts();
        $post = $posts->find()->where(['id' => $id])->one();
        $img = new UploadForm();
